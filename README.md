...
Creates a public HTTPS tunnel so Shopify can redirect back to your machine after OAuth.
...
The tool works with apps already created in Shopify Partner Dashboard. Have these ready:
...
The tool checks that ngrok is installed, then starts a tunnel on port 5000. After a few seconds you will see a public URL like `https://xxxx.ngrok-free.app`.

**Requires:** ngrok installed and auth token configured.
...
The tool writes a `shopify.app.toml` file with your tunnel URL as the app and redirect URL, then deploys it to Shopify Partner Dashboard automatically using the `--force` flag to skip confirmation.

This tells Shopify to accept the OAuth redirect to your tunnel URL. **This step must succeed** — if it fails, Shopify will block the OAuth with a `redirect_uri is not whitelisted` error.

**Requires:** Shopify CLI installed and logged in.
...
```
https://xxxx.ngrok-free.app/auth/install?shop=mystore.myshopify.com
```

Copy and open this in your browser. Shopify shows an install screen — click **Install** or **Update**. Shopify then redirects back to the tool automatically.
...
- Client Secret
- Scopes
- **Permanent Access Token** (highlighted in green)
...
| Error | Cause | Fix |
|---|---|---|
| `ngrok not found` | ngrok not installed or not in PATH | Install ngrok, restart terminal |
| Tunnel URL not found | ngrok auth token missing | Run `ngrok config add-authtoken YOUR_TOKEN` |
...
| `redirect_uri is not whitelisted` | Deploy step failed | Fix the deploy error and re-run the tool |
...
| Port 5000 already in use | Another process on port 5000 | Kill the other process or restart machine |

***

## Important notes

- **Run the tool fresh each time** — ngrok generates a new URL on every run (free plan), so the deploy step re-registers it automatically each session.
